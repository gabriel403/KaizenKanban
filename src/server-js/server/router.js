...
Router.prototype.rest 				= function(request, payload){
	var self 						= this;
	...
	if ( request.method in obj ) {
		...
		obj[request.method](self.parsedUrl, payload);
	} else {
...
Router.prototype.default 			= function(request, payload) {
	var self 						= this;
	var filepath 					= sl.get('config').get('topDir') + self.path;
	...
	sl.get('fs').readFile(filepath, self.pageencoding, function(err, data){
		if (err) {
			err.code 	= 404;
...
Router.prototype.handle 			= function(request, payload){
	var self 		= this;
	...
	self[self.route.callback](request, payload);
}
...
var route 				= function(request, payload, callback) {
	var router 			= new Router();

	var d 				= sl.get('domain').create();
	...
	d.run(function(){router.handle(request, payload);});
}
