define(["dojo/_base/declare", "dojo/query", "dojo/dom-style", "dojo/aspect", "dojo/dom-attr", "dojo/on",
	 ...
	 "dojo/_base/window", "dijit/registry", "dojo/NodeList-traverse" ],
	 ...
	 win, registry){
		return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
			...
			postCreate:         function() {
				...
				this.detailsDialog = registry.byId('detailsDialog');
			...
			processCards: function(kbcSource, cards) {
				...
				kbcSource.dndSource.getAllNodes().forEach(function(node){
					query('.zoom_icon', node).forEach(function(item, index){
						console.log(item);
							on(item, 'click',
								lang.hitch(this, function(e){
									var nodeData = kbcSource.dndSource.getItem(node.id).data;
									this.detailsDialog.set("content", JSON.stringify(nodeData));
									// domAttr.set(this.detailsDialogInner, 'innerHTML', node.id);
									this.detailsDialog.show();
								})
							);
						}, this);
        			// console.log(kbcSource.dndSource.getItem(node.id).data);
    			}, this);
